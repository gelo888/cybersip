...
import { Building2, Loader2, AlertCircle, Plus, Pencil, Trash2 } from "lucide-react"
...
export function CompaniesTable() {
  ...
  const { pageData, page: safePage, pageCount, total, start } = usePagination(companies.data, page, pageSize)
  ...
  return (
    <section className="space-y-3">
                ...
                <tr key={company.id} className="border-b last:border-b-0 hover:bg-muted/20 transition-colors">
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <Button variant="ghost" size="icon-xs" onClick={() => openEdit(company)}>
                        <Pencil className="size-3.5" />
                      </Button>
                      <Button variant="ghost" size="icon-xs" onClick={() => setDeleteTarget(company)}>
      ...
      <CompanyFormDialog open={formOpen} onOpenChange={setFormOpen} company={editTarget} />
