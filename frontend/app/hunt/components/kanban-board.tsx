...
function pendingOurValueForCompany(
    contracts: Contract[],
    companyId: string,
): number {
    let sum = 0;
...
export function KanbanBoard() {
    ...
    function handleDragStart(event: DragStartEvent) {
        const eng = event.active.data.current?.engagement as
            | Engagement
            | undefined;
        ...
        return (
            <div className="flex flex-col items-center justify-center px-6 py-20 text-center">
                <p className="text-muted-foreground mb-4">
                    No pipeline stages configured yet. Create stages to start
    ...
    return (
        <>
                ...
                <div className="mx-6 mb-4 flex flex-col gap-2 rounded-lg border bg-muted/30 px-4 py-3 text-sm">
                    ...
                    {staleEngagementLink && (
                        <p className="text-xs text-sophos-orange mb-0">
                            Linked engagement is no longer on the board; showing
                            all deals for this company.
                        </p>
                    )}
                </div>
            )}
                ...
                <div className="flex flex-wrap items-center gap-6 sm:gap-8">
                    <div>
                        ...
                        <p className="text-muted-foreground/80 m-0 mt-0.5 max-w-56 text-[10px]">
                            Pending our_contract totals, one per company on the
                            board
                        </p>
                    </div>
                    <div
                        className="bg-border/50 hidden h-10 w-px sm:block"
                        aria-hidden
                    />
                    <div>
                        <p className="text-muted-foreground m-0 text-[10px] font-bold tracking-wider uppercase">
                ...
                <div className="kanban-scroll-x min-h-[min(520px,calc(100dvh-16rem))] flex-1 overflow-x-auto px-6 pb-6">
                    <div className="inline-flex h-full min-h-[inherit] min-w-max items-stretch gap-6">
                        {stages.map((stage, stageIndex) => {
                            const colEngagements =
                                engagementsByStage.get(stage.id) ?? [];
                            ...
                            const borderAccent =
                                STAGE_BORDER[
                                    stageIndex % STAGE_BORDER.length
                                ] ?? "border-l-primary";

                            return (
                                <div
                                    key={stage.id}
                                    className="flex w-90 shrink-0 flex-col self-stretch"
                                >
            ...
            <DeleteConfirmDialog
                ...
                entityName={
                    deleteTarget
                        ? `${deleteTarget.type} with ${deleteTarget.company_name}`
                        : ""
                }
